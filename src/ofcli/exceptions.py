...
async def internal_exception_handler(request: Request, exc: InternalException):
    """Replacement callback for handling InternalException exceptions.

    :param request: request object that caused the InternalException
    :param exc: InternalException containing the error message
    """
    _ = request
    return JSONResponse(
        status_code=HTTP_500_INTERNAL_SERVER_ERROR,
